...
time_t input_port_init(running_machine &machine)
{
	...
	// movie rerecording
	machine.save().save_memory("inptport", NULL, 0, "movie_header",
	                           &portdata->movie_header,
	                           sizeof(portdata->movie_header[0]),
	                           sizeof(portdata->movie_header));
	machine.save().save_memory("inptport", NULL, 0, "current_frame",
...
void movie_postsave(running_machine &machine, emu_file *file)
{
	file->write(movie.buffer, machine.input_port_data->bytes_per_frame*(machine.input_port_data->current_frame+1));
}

void movie_postload(running_machine &machine, emu_file *file)
{
	input_port_private *portdata = machine.input_port_data;

	reserve_movie_buffer_space((portdata->bytes_per_frame*(portdata->current_frame+1)) - (movie.pointer - movie.buffer));
	file->read(movie.buffer, portdata->bytes_per_frame*(portdata->current_frame+1));
...
static void playback_open_file(running_machine &machine,const char* filename)
{
	...
	if (memcmp(machine.system().name, portdata->movie_header + 0x0c, strlen(machine.system().name) + 1) != 0)
		fatalerror("Input file is for " GAMENOUN " '%s', not for current " GAMENOUN " '%s'.\n", portdata->movie_header + 0x0c, machine.system().name);
...
static void playback_init(running_machine &machine)
{
	...
	strncpy(filename,machine.options().playback(),_MAX_PATH);
...
static void record_open_file(running_machine &machine,const char* filename)
{
	...
	strcpy((char *)portdata->movie_header + 0x0c, machine.system().name);
...
static void record_init(running_machine &machine)
{
	...
	strncpy(filename,machine.options().record(),_MAX_PATH);
...
UINT32 get_port_digital(input_port_config &port)
{
...
void set_port_digital(input_port_config &port, UINT32 new_digital)
{
